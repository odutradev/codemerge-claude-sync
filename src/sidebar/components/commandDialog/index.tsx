...
export const CommandDialog = ({ cmdDialogOpen, setCmdDialogOpen, cmdLoading, cmdOutput, handleFetchCommandOutput, handleInjectOutput }: CommandDialogProps) => (
    <StyledDialog open={cmdDialogOpen} onClose={() => setCmdDialogOpen(false)} maxWidth="md" fullWidth>
        <StyledDialogTitle>
            ...
            <IconButton size="small" onClick={() => setCmdDialogOpen(false)}><MdClose size={20} /></IconButton>
        </StyledDialogTitle>
        <DialogContent dividers>
            {cmdLoading ? <LoaderContainer><CircularProgress /></LoaderContainer> : (
                <Box>
                    {cmdOutput?.status === 'no_command_executed' ? <Alert severity="info" variant="outlined">Nenhum comando configurado ou executado recentemente.</Alert> : (
                        <>
                            <InfoGrid>
                                <Box><Typography variant="caption" color="text.secondary">Comando</Typography><CommandText variant="body2">{cmdOutput?.command ?? '-'}</CommandText></Box>
                                <Box><Typography variant="caption" color="text.secondary">Horário</Typography><Typography variant="body2">{cmdOutput?.timestamp ? new Date(cmdOutput.timestamp).toLocaleTimeString() : '-'}</Typography></Box>
                                <Box><Typography variant="caption" color="text.secondary">Status</Typography><Typography variant="body2" color={cmdOutput?.success ? 'success.main' : 'error.main'} fontWeight="bold">{cmdOutput?.success ? 'SUCESSO' : 'ERRO'}</Typography></Box>
                            </InfoGrid>
