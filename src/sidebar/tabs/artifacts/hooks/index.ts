...
export const useArtifacts = (fetchViaBackground: FetchViaBackground): UseArtifactsReturn => {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            ...
            setSelectedDeletions(new Set(snap.selectedDeletions));
            setSelectedCommands(new Set(snap.selectedCommands));
            setCommitMessage(snap.commitMessage);
            setSelectedIndices(new Set(snap.selectedIndices));
    ...
    const handleFetchArtifacts = useCallback(async (silent = false) => {
        ...
        try {
            ...
            filteredArtifacts.forEach((art: Artifact, i: number) => {
                if (!art.name.toLowerCase().endsWith('.md')) {
    ...
    const handleExecuteCommands = async () => {
        ...
        try {
            const res = await fetchViaBackground(`${serverUrl}/execute-commands`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    basePath: './',
                    commandsToExecute: Array.from(selectedCommands)
                })
            });

            if (!res.success) {
                throw new Error(`Execução: ${res.error}`);
            }
    ...
    const handleApplyAll = async () => {
        ...
        try {
            ...
            if (selectedIndices.size > 0) {
                const selectedFiles = Array.from(selectedIndices).map(i => ({
                    path: artifacts[i].name,
                    content: removeComments ? processCode(artifacts[i].code, { removeComments: true, removeEmptyLines, removeLogs }) : artifacts[i].code
                }));

                tasks.push(
                    fetchViaBackground(`${serverUrl}/upsert`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
            ...
            if (selectedDeletions.size > 0) {
                tasks.push(
                    fetchViaBackground(`${serverUrl}/delete-files`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            basePath: './',
                            files: Array.from(selectedDeletions)
                        })
                    })
                );
            ...
            setActionLoading(false);
            setTimeout(() => {
                setHookStatus(p => p !== 'loading' ? 'idle' : p);
            }, 4000);
        }
    };
    ...
    return {
        ...
        actions: {
            ...
            toggleDeleteSelection: (p: string) => {
                setSelectedDeletions(pr => {
                    const n = new Set(pr);
                    n.has(p) ? n.delete(p) : n.add(p);
                    return n;
                });
            },
            toggleCommandSelection: (c: string) => {
                setSelectedCommands(p => {
                    const n = new Set(p);
