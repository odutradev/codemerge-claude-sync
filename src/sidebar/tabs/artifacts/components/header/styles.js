...
export const getStatusProps = (status, isChecking) => {
...
export const iconButtonStyles = (isActive, hookStatus = 'idle') => {
    const isSuccess = hookStatus === 'success';
    const isError = hookStatus === 'error';

    return {
        border: '1px solid',
        borderColor: isSuccess ? 'success.main' : isError ? 'error.main' : isActive ? 'primary.main' : 'divider',
        borderRadius: 2,
        width: 36,
        height: 36,
        flexShrink: 0,
        animation: isSuccess ? `${pulseGreen} 1s infinite` : isError ? `${pulseRed} 1s infinite` : 'none',
        color: isSuccess ? 'success.main' : isError ? 'error.main' : 'inherit',
