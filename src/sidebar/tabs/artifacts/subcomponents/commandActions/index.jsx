...
export const CommandActions = ({ commandsToExecute, selectedCommands, toggleCommandSelection, handleExecuteCommands, actionLoading, serverStatus }) => {
    if (!commandsToExecute || commandsToExecute.length === 0) return null;

    const hasSelection = selectedCommands.size > 0;
                    ...
                    {commandsToExecute.map((cmd, index) => (
                        <ListItem key={`cmd-${index}`} button onClick={() => toggleCommandSelection(cmd)} sx={listItemStyles(selectedCommands.has(cmd))}>
                            <Box sx={{ display: 'flex', alignItems: 'center', minWidth: 0, width: '100%' }}>
                                <Checkbox checked={selectedCommands.has(cmd)} size="small" sx={{ p: 0.5, mr: 1.5, color: 'info.main', '&.Mui-checked': { color: 'info.main' } }} />
                                <Box sx={{ flexGrow: 1, minWidth: 0, mr: 2 }}>
                                    <Tooltip title={cmd} placement="top-start" enterDelay={500}>
                                        <Typography variant="body2" noWrap sx={{ color: 'text.primary', fontFamily: 'monospace', fontSize: '0.75rem' }}>
                                            $ {cmd}
                                        </Typography>
                                    </Tooltip>
                                </Box>
