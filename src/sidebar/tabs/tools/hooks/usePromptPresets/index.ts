...
const usePromptPresets = (): UsePromptPresetsReturn => {
  const { showNotification } = useNotificationStore()
  const { presets, addPreset, updatePreset, deletePreset } = usePromptStore()

  const [formData, setFormData] = useState<{ id: string | null; title: string; prompt: string }>({
    id: null,
    title: '',
    prompt: ''
  })
  ...
  const handleOpenDialog = useCallback((preset: Preset | null = null) => {
    setFormData(
      preset || {
        id: null,
        title: '',
  ...
  const handleSave = useCallback(() => {
    if (!formData.title.trim() || !formData.prompt.trim()) {
      return showNotification('Título e prompt são obrigatórios', 'warning')
    }

    if (formData.id) {
      updatePreset(formData.id, {
