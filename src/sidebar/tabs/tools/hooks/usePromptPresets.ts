...
export const usePromptPresets = (showNotification: (text: string, type: MessageState['type']) => void) => {
    ...
    const handleOpenDialog = useCallback((preset: { id: string | null; title: string; prompt: string } | null = null) => { setFormData(preset || { id: null, title: '', prompt: '' }); setDialogOpen(true); }, []);
